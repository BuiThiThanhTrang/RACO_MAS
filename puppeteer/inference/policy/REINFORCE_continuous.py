...
class ContinuousREINFORCE(LearningPolicy):
    def __init__(self, agent_graph, action_graph, config, runtime_config):
        ...
        self.policy_mode = self.config.get("policy_mode", "train" if self.config["training"]["training"] else "evolved")
        ...
        # Initialize state representation and policy network. Supported modes:
        # - api_embedding: call an embedding API for a task-dependent state.
        # - local_reward_model: load the 70B reward model from model_weight_path.
        self.state_representation = self._build_state_representation()
        ...
        load_policy = self.config.get("paths", {}).get("load_policy", None)
        if load_policy is None:
            load_policy = self.loading or (not self.training and self.policy_mode != "initialized")
        if load_policy:
            model_path = self.model_path if self.loading and self.model_path else self.get_latest_model_path()
            if model_path:
                self.load_model(model_path)
            else:
                logger.warning("Policy loading requested, but no checkpoint was found. Continuing with initialized policy.")
        else:
            logger.info("Policy checkpoint loading disabled. Using initialized policy weights.")
    ...
    def _build_state_representation(self):
        ...
        if state_type not in supported_state_types:
            raise ValueError(
                "config/global.yaml must set state_representation.type to one of "
                f"{sorted(supported_state_types)}. Dummy/random state representation has been removed."
            )
        ...
        if not isinstance(model_weight_path, str) or not model_weight_path.strip():
            raise ValueError(
                ...
                "Set model_weight_path to the 70B reward model, for example "
                "'nvidia/Llama-3.1-Nemotron-70B-Reward-HF', or switch "
                "state_representation.type to api_embedding."
            )
